    ...
    companion object {
        ...
        fun globalUser(userId: Long): User {
            val map = mutableMapOf<String, Any?>(
                Pair("type", GLOBAL_USER),
                Pair("userId", userId)
            )
            HibernateFactory.selectOne(User::class.java, map)?.let {
                return it
            } ?: return User(type = GLOBAL_USER, userId = userId)
        }
        ...
        fun group(groupId: Long): User {
            ...
            HibernateFactory.selectOne(User::class.java, map)?.let {
                return it
            } ?: return User(type = GROUP, groupId = groupId)
        }
        ...
        fun groupAdmin(groupId: Long): User {
            val map = mutableMapOf<String, Any?>(
                Pair("type", GROUP_ADMIN),
                Pair("groupId", groupId)
            )
            HibernateFactory.selectOne(User::class.java, map)?.let {
                return it
            } ?: return User(type = GROUP_ADMIN, groupId = groupId)
        }
        ...
        fun member(groupId: Long, userId: Long): User {
            val map = mutableMapOf<String, Any?>(
                Pair("type", GROUP_MEMBER),
                Pair("groupId", groupId),
                Pair("userId", userId)
            )
            HibernateFactory.selectOne(User::class.java, map)?.let {
                return it
            } ?: return User(type = GROUP_MEMBER, groupId = groupId, userId = userId)
        }
